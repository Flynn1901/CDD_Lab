...
module mp_adder #(
    ...
    always @(posedge iClk)
    begin
        if(iRst==1)     regA_Q <= { OPERAND_WIDTH{1'b0} }; //clean & assign all zeros
        else            regA_Q <= regA_D;//shifted version of last operand
    end
    ...
    assign muxA_Out = (muxA_sel == 0) ? iOpA : { {ADDER_WIDTH{1'b0}}  , regA_Q[OPERAND_WIDTH-1:ADDER_WIDTH]};//all zeros at MSB and concatenate for shifting right
